...
export default function Form({
  ...
  const formatDate = (examDate, examTime) => {
    ...
    const timeString = examTime + ":00";
    return dateString + "T" + timeString + "Z";
  };

  const submitForm = async (e) => {
    e.preventDefault();
    console.log("formatted date is",new Date(formatDate(dateOfExam, paperTime)));
    ...
    const res = await axios.post(
      `http://localhost:3000/api/faculty/paper_creation/${
        edit ? "edit_paper" : "new_paper"
      }`,
      {
        paper_id: examDetails ? examDetails.paper_id : null,
        paper_name: paperName,
        time: paperTime + ":00",
        date: new Date(formatDate(dateOfExam, paperTime)),
