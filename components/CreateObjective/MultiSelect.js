...
const MultiSelectDropdown = ({ options, onChange }) => {
    ...
    return (
        <div className="relative">
            ...
            {dropdown &&
                <div
                    className="absolute z-10 mt-2 w-full bg-white rounded border border-primary-black border-opacity-[0.15]"
                >
                    <ul className="list-reset">
                        {options.map((option) => (
                            <li
                                key={option}
                                className={`p-2 cursor-pointer hover:bg-blue-800 hover:text-white ${selectedOptions.includes(option)
