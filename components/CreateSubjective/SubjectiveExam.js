...
const SubjectiveExam = ({
  ...
  useEffect(() => {
    if (
      Array.isArray(subjectivesLocal) &&
  ...
  const handleAnswerChange = (e) => {
    setCurrentQuestion(
      btn_call === "Create Question"
        ? { ...currentQuestion, correct_answer: e.target.value }
  ...
  async function addQuestion(i, question) {
    console.log(
      "[" + i + "] - " + "current question in addQuestion: ",
      question
    );
  ...
  const deleteCurrentQuestions = async (subjectives_ids_array) => {
    try {
      console.log(
        "subjectives_ids_array in deleteCurrentQuestions: ",
        subjectives_ids_array
      );
  ...
  async function handleRegenQuestions() {
    ...
    console.log(
      "sq_ids of current questions: ",
      subjectivesLocal.map((sub) => {
        return sub.sq_id;
      })
    );

    const subs_to_regen = subjectivesLocal.filter((sub) => {
      return sub.checked === true;
    });
    const subs_to_regen_sq_ids = subs_to_regen.map((sub) => {
      return sub.sq_id;
    });
  ...
  function handleSelectMCQ(input_index) {
    ...
    console.log(
      "checkedsubjectivesLocal in handleSelectMCQ: ",
      checkedsubjectivesLocal
    );
    checkedsubjectivesLocal[input_index].checked =
      !checkedsubjectivesLocal[input_index].checked;
  ...
  const handleGetQuestions = async () => {
    ...
    try {
      ...
      if (subjectives_ids_array.length > 0) {
        deleteCurrentQuestions(subjectives_ids_array);
      }
      setAdding(false);
      setControl(false);
      setControl_2(true);
  ...
  async function handleAddSubjective() {
    ...
    console.log(
      "currentQuestion in handleAddSubjective before: ",
      currentQuestion
    );
    console.log(
      "currentQuestion.questionnumber in handleAddSubjective before: ",
      subjectivesLocal.length
    );
    console.log(
      "subjectivesLocal in handleAddSubjective before: ",
      subjectivesLocal
    );
    console.log(
      "subjective_questions in handleAddSubjective before: ",
      subjective_questions
    );
    ...
    try {
      ...
      console.log(
        "updatedSubjectiveQuestions in handleAddSubjective: ",
        updatedSubjectiveQuestions
      );
      ...
      console.log(
        "currentQuestion in handleAddSubjective after: ",
        currentQuestion
      );
      console.log(
        "currentQuestion.questionnumber in handleAddSubjective after: ",
        subjectivesLocal.length
      );
      console.log(
        "subjectivesLocal in handleAddSubjective after: ",
        subjectivesLocal
      );
      console.log(
        "subjective_questions in handleAddSubjective after: ",
        subjective_questions
      );
    } catch (err) {
      console.log("err: ", err);
  ...
  const handleUpdateSubjective = async (question) => {
    ...
    try {
      ...
      console.log(
        "subjectivesLocal in handleUpdateSubjective: ",
        subjectivesLocal
      );
      ...
      console.log(
        "updatedSubjectives in handleUpdateSubjective: ",
        updatedSubjectives
      );
  ...
  const handleDeleteSubjective = async (sq_id, parent, isChild = false) => {
    ...
    if (deletedSubjective.status === 200) {
      ...
      if (isChild) {
        ...
        if (btn_call === "Create Question") {
          index = newMCQs.findIndex((subjective) => subjective.id === sq_id);
        } else {
          index = newMCQs.findIndex((subjective) => subjective.sq_id === sq_id);
        }
  ...
  return (
    <div className="flex font-poppins flex-col items-center p-6">
      ...
      {control_3 && (
        <Replace_Modal
          mcqs={subjectivesLocal}
          setMCQs={setSubjectivesLocal}
      ...
      {!editing && (
        <div className="w-4/12 flex flex-col justify-center gap-y-4">
          ...
          >
            {btn_call === "Generate Random Paper"
              ? "Load Subjectives from Data Bank"
              : "Add Subjective"}
          </button>
      ...
      {control && (
        <div className="w-full p-10 bg-slate-100 mt-6 rounded-2xl flex flex-col justify-center">
          <div className="flex justify-between">
            <h2 className="text-xl font-bold mb-4">Add Subjective Questions</h2>
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        <button
                          ...
                          onClick={(event) => {
                            handleReplaceBtn(event, subjective);
                          }}
                        >
                          Replace
                        </button>
