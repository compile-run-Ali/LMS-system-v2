...
export default function DashboardComponent({exams_data, paperapproval_data}) {
  ...
  useEffect(() => {
    if (exams_data !== undefined && exams_data !== null && exams_data.length > 0) {
      setCourses(exams_data)
    }
    if (paperapproval_data !== undefined && paperapproval_data !== null && paperapproval_data.length > 0) {
  ...
  return (
    <div>
      ...
      {paperapproval_data !== undefined && paperapproval_data !== null && paperapproval_data.length > 0 && (
        <div className='pr-10 pl-5 mt-10'>
          <h1 className='text-2xl font-poppins font-bold'>Submitted For Approval:</h1>
